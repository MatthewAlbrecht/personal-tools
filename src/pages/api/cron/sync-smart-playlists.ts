...
import { syncAllSmartPlaylistsForUser } from "~/lib/smart-playlists-sync";
...
export default async function handler(
	...
	try {
		console.log("🔄 Starting scheduled smart playlist sync for user:", userId);
		...
		const result = await syncAllSmartPlaylistsForUser({
			accessToken,
			userId,
		});

		console.log(
			"✅ Smart playlist sync completed:",
		...
		console.error("❌ Cron smart playlist sync failed:", error);

		return res.status(500).json({
			error: "Smart playlist sync failed",

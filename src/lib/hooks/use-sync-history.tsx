  ...
  const syncHistory = useCallback(async (): Promise<SyncStats | null> => {
    ...
    try {
      ...
      const lines = [
        ...
        data.albumsFetchFailed > 0 ? `  • ${data.albumsFetchFailed} failed to fetch` : null,
        data.tracksBackfilledFromAlbums > 0 ? `  • ${data.tracksBackfilledFromAlbums} tracks backfilled` : null,
        '',
        '🎧 Listens:',
      ...
      if (data.newAlbumNames?.length > 0) {
        lines.push('', '🆕 New albums:', ...data.newAlbumNames.map((n) => `  • ${n}`));
      }
      if (data.recordedListenAlbumNames?.length > 0) {
        lines.push('', '✅ Listened to:', ...data.recordedListenAlbumNames.map((n) => `  • ${n}`));
      }
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to sync history');
      return null;
    } finally {
      setIsSyncing(false);
    }

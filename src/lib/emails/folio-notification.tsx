...
export function FolioNotificationEmail({ newReleases, syncResult }: FolioNotificationEmailProps) {
  return (
    <Html lang="en">
      <Head />
      <Body style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#ffffff' }}>
        <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
          ...
          <Text style={{ fontSize: '16px', color: '#374151', marginBottom: '20px' }}>
            Great news! {newReleases.length} new release{newReleases.length > 1 ? 's have' : ' has'} been found during the latest sync.
          </Text>
          ...
          <Section style={{
            ...
            <Text style={{ fontSize: '14px', margin: '0' }}>
              <strong>Total synced:</strong> {syncResult.syncedCount} • <strong>New releases:</strong> {newReleases.length}
          ...
          {newReleases.map((release) => (
            <Section key={release.id} style={{
              ...
              <Text style={{
                ...
                <strong>ID:</strong> {release.id} • <strong>SKU:</strong> {release.sku} • <strong>Price:</strong> {formatPrice(release.price)}
              </Text>

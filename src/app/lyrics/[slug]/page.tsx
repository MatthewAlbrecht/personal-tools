...
function formatLyrics(lyrics: string): React.ReactNode {
  ...
  return lines.map((line, index) => {
    ...
    if (isSectionHeader) {
      return (
        <span key={index} className="text-muted-foreground text-sm print:text-xs">
          {line}
          <br />
        </span>
      );
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 print:px-2 print:py-4">
      ...
      <header className="mb-8 text-center print:mb-12">
        ...
        <h2 className="text-muted-foreground text-2xl print:text-3xl">
          {album.artistName}
        </h2>
      ...
      <div className="mt-12 border-t pt-6 text-center text-muted-foreground text-sm no-print">
        <p>
          Lyrics sourced from{" "}
          <a

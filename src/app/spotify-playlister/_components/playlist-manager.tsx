...
export function PlaylistManager({
	...
	const [newlyCreatedPlaylist, setNewlyCreatedPlaylist] = useState<{
	...
	const localPlaylistIds = new Set(
		localPlaylists?.map((p) => p.spotifyPlaylistId) ?? [],
	);
	...
	function handleAddPlaylist() {
		if (selectedSpotifyPlaylist && addGeneratedDescription.trim()) {
			onAddPlaylist(
				...
				addGeneratedDescription.trim(),
				addUserNotes.trim() || undefined,
			);
			setIsAdding(false);
	...
	function handleSaveEdit(playlist: LocalPlaylist) {
		const newDescription =
			editGeneratedDescription.trim() || playlist.description;
		onUpdateDescription(
			playlist._id,
			newDescription,
			editUserNotes.trim() || undefined,
		);
		setEditingId(null);
		setEditUserNotes("");
	...
	async function handleConfirmBackfill(matches: BackfillMatch[]) {
		...
		if (!selectedPlaylist || !onConfirmBackfill) {
			console.error(
				"[PlaylistManager] Missing selectedPlaylist or onConfirmBackfill",
			);
			return;
		}
		...
		try {
			...
			toast.success(
				`Added ${matches.length} songs to "${selectedPlaylist.name}"`,
			);
			setPreviewDialog({
				isOpen: false,
				matches: [],
			...
			toast.error(
				`Failed to add songs: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		} finally {
	...
	return (
		<Card>
			...
			<CardContent className="space-y-4">
				...
				{isCreating && (
					<div className="space-y-3 rounded-lg border border-primary/50 border-dashed bg-primary/5 p-4">
						...
						<div className="space-y-2">
							...
							<p className="text-muted-foreground text-xs">
								Enter short notes, then click{" "}
								<Sparkles className="inline h-3 w-3" /> to generate a
								description
							</p>
						</div>
					...
					<div className="space-y-3 rounded-lg border border-green-500/50 bg-green-500/10 p-4">
						<div className="flex items-center gap-2">
							...
							<p className="font-medium text-sm">
								Created &quot;
								{formatPlaylistName(newlyCreatedPlaylist.playlist.name)}&quot;!
							</p>
						</div>
						<p className="text-muted-foreground text-sm">
							Want to seed this playlist with songs you&apos;ve already
							categorized?
						</p>
						<div className="flex gap-2">
							<Button
							...
							<Button
								size="sm"
								variant="ghost"
								onClick={handleDismissNewPlaylist}
							>
								Skip
							</Button>
						</div>
				...
				{isAdding && (
					<div className="space-y-3 rounded-lg border border-dashed p-4">
						...
						<select
							...
							onChange={(e) => {
								const playlist = spotifyPlaylists?.find(
						...
						>
							...
							{availableSpotifyPlaylists?.map((playlist) => (
								<option key={playlist.id} value={playlist.id}>
									{formatPlaylistName(playlist.name)} ({playlist.tracks.total}{" "}
									tracks)
								</option>
							))}
						</select>
						...
						<div className="flex gap-2">
							<Button
								...
								disabled={
									!selectedSpotifyPlaylist || !addGeneratedDescription.trim()
								}
							>
								<Plus className="mr-2 h-4 w-4" />
								Add
				...
				{!localPlaylists?.length ? (
					<div className="py-8 text-center">
						...
						<p className="mt-1 text-muted-foreground text-sm">
							Add Spotify playlists with mood descriptions to start categorizing
							songs.
						</p>
					</div>
				) : (
									...
									<div className="space-y-3">
										...
										<div className="flex gap-2">
											<Button
												...
												disabled={
													!editGeneratedDescription &&
													editUserNotes === (playlist.userNotes ?? "")
												}
											>
										...
										<div className="flex gap-1">
											...
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														{playlist.isActive ? "Disable" : "Enable"} for
														suggestions
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>

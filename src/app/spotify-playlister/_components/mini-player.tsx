...
export function MiniPlayer({
  ...
  const isPlaying = playerState.isPlaying && playerState.currentTrackId === currentTrack.id;
  const albumImage = currentTrack.album.images[currentTrack.album.images.length - 1]?.url;
  ...
  return (
    <div className="fixed right-4 bottom-4 z-50 flex items-center gap-3 rounded-lg border bg-background/95 p-3 shadow-lg backdrop-blur-sm">
      ...
      <div className="min-w-0 max-w-[160px]">
        <p className={`truncate font-medium text-sm ${isPlaying ? 'text-green-500' : ''}`}>
          {currentTrack.name}
        </p>

...
import { Check, X, Loader2, ListMusic, ChevronDown, ChevronUp } from 'lucide-react';
            ...
            {suggestions.map((suggestion) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 rounded-lg border p-3 transition-colors ${isSelected ? 'border-primary bg-primary/5' : 'border-border'
                    }`}
                >
                  <Checkbox
                    id={suggestion.playlistId}
                    checked={isSelected}
                    onCheckedChange={() => togglePlaylist(suggestion.playlistId)}
                    disabled={isSaving}
                  />
            ...
            {showOtherPlaylists && (
              <div className="mt-2 flex flex-wrap gap-2">
                {otherPlaylists.map((playlist) => {
                  const isSelected = selectedIds.has(playlist.spotifyPlaylistId);
                  return (
                    <button

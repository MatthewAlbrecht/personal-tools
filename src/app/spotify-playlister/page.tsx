...
export default function SpotifyPlaylisterPage() {
	...
	const { isSyncing: isSyncingHistory, syncHistory: handleSyncHistory } =
		useSyncHistory({
			userId,
			isConnected,
			getValidAccessToken,
	...
	const updatePlaylistDescription = useMutation(
		convexApi.spotify.updatePlaylistDescription,
	);
	const togglePlaylistActive = useMutation(
		convexApi.spotify.togglePlaylistActive,
	);
	...
	const savePendingSuggestions = useMutation(
		convexApi.spotify.savePendingSuggestions,
	);
	const clearPendingSuggestions = useMutation(
		convexApi.spotify.clearPendingSuggestions,
	);
	const upsertTracksFromRecentlyPlayed = useMutation(
		convexApi.spotify.upsertTracksFromRecentlyPlayed,
	);
	const upsertTracksFromLiked = useMutation(
		convexApi.spotify.upsertTracksFromLiked,
	);
	...
	const [selectedTrack, setSelectedTrack] = useState<RecentlyPlayedItem | null>(
		null,
	);
	...
	const [pendingSuggestionsCache, setPendingSuggestionsCache] = useState<
		Map<string, { userInput: string; suggestions: PlaylistSuggestion[] }>
	>(new Map());
	...
	const recentTracks: RecentlyPlayedItem[] | undefined = (() => {
		...
		if (currentlyPlayingTrack) {
			const isAlreadyInList = dbTracks.some(
				(t) => t.track.id === currentlyPlayingTrack.id,
			);
			if (!isAlreadyInList) {
				return [
	...
	const {
		...
		togglePlayback,
		skip,
	} = useSpotifyPlayer(getValidAccessToken, isConnected);
			...
			if (currentlyPlayingRes.ok) {
				...
				if (cpData.currentlyPlaying?.item) {
					setCurrentlyPlayingTrack(
						cpData.currentlyPlaying.item as SpotifyTrack,
					);
		...
		if (getPendingSuggestions && selectedTrack) {
			...
			if (!cached && getPendingSuggestions) {
				setPendingSuggestionsCache((prev) =>
					new Map(prev).set(selectedTrack.track.id, {
						userInput: getPendingSuggestions.userInput,
						suggestions:
							getPendingSuggestions.suggestions as PlaylistSuggestion[],
					}),
				);
			}
		}
	...
	const currentPlayingTrack = (() => {
		...
		if (selectedTrack?.track.id === playerState.currentTrackId)
	...
	function handleSelectTrack(item: RecentlyPlayedItem) {
		...
		if (
			selectedTrack?.track.id === item.track.id &&
	...
	function createMinimalTrack(saved: {
		...
		return {
			...
			duration_ms: 0,
			external_urls: {
				spotify: `https://open.spotify.com/track/${saved.trackId}`,
			},
			preview_url: null,
			track_number: 0,
		};
	...
	async function handleCreatePlaylist(
		...
		description: string,
		userNotes?: string,
	): Promise<{ playlist: SpotifyPlaylist } | null> {
		...
		try {
			...
			if (!res.ok) {
				...
				throw new Error(
					errorData.error || "Failed to create playlist on Spotify",
				);
			}
	...
	async function handleCheckPastSongs(
		playlist: LocalPlaylist,
	): Promise<BackfillMatch[] | null> {
		...
		try {
			...
			const data = await res.json();
			console.log(
				"[Backfill] API returned matches:",
				data.matches?.length || 0,
			);
			console.log(
				"[Backfill] Sample matches:",
				data.matches?.slice(0, 3).map((m: BackfillMatch) => ({
					trackId: m.trackId,
	...
	async function handleConfirmBackfill(
		...
		console.log(
			"[Backfill] Starting confirm for playlist:",
			playlist.name,
			"matches:",
			matches.length,
		);
		...
		if (!categorizations?.length || !matches.length) {
			console.warn(
				"[Backfill] Early exit - categorizations:",
		...
		try {
			...
			const invalidMatches = matches.filter(
				(m) => !isValidSpotifyId(m.trackId),
			);

			if (invalidMatches.length > 0) {
				console.error(
					"[Backfill] Found invalid Spotify track IDs:",
				...
				toast.error(
					`Cannot add ${invalidMatches.length} track(s) with invalid IDs`,
				);
				return;
			}
			...
			for (let i = 0; i < trackUris.length; i += 100) {
				...
				console.log(
					`[Backfill] Adding batch ${batchNum}/${Math.ceil(trackUris.length / 100)} with ${batch.length} tracks`,
				);
				...
				console.log(
					`[Backfill] Batch ${batchNum} response status:`,
					res.status,
				);
				...
				if (!res.ok) {
					const errorText = await res.text();
					console.error(
						`[Backfill] Batch ${batchNum} error response:`,
						errorText,
					);
					throw new Error(
						`Failed to add tracks to Spotify (batch ${batchNum}): ${res.status}`,
					);
				}
			...
			console.log(
				"[Backfill] Successfully added",
				totalAdded,
				"songs to playlist",
			);
			toast.success(
				`Added ${matches.length} songs to "${formatPlaylistName(playlist.name)}"`,
			);
		} catch (error) {
			console.error("[Backfill] Error during confirm:", error);
			toast.error(
				`Failed to add songs to playlist: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
					...
					<div className="space-y-6">
						...
						{selectedTrack && reviewState.status !== "idle" && (
							<>
								...
								{(reviewState.status === "suggestions" ||

  ...
  for (const container of containers) {
    ...
    function processNode(node: any, depth: number = 0): void {
      if (node.nodeType === 3) { // Text node
        ...
        const excludeFromSelection = node.getAttribute?.('data-exclude-from-selection');
        
        // Skip elements marked as excluded or containing header/UI content
        if (excludeFromSelection === 'true' || 
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!initialResponse.ok) {
      ...
      console.error('Failed to fetch:', initialResponse.status, errorText.substring(0, 200));
      return NextResponse.json(
    ...
    const artistNameElem = root.querySelector('[class*="SongHeader"][class*="CreditList"] a');
    const artistName = artistNameElem ? artistNameElem.textContent.trim() : '';
    
    const albumUrl = albumTitleElem ? albumTitleElem.getAttribute('href') || '' : '';
    const fullAlbumUrl = albumUrl.startsWith('http') ? albumUrl : `https://genius.com${albumUrl}`;
    ...
    if (fullAlbumUrl) {
      ...
      if (albumResponse.ok) {
        ...
        const tracklistContainer = albumRoot.querySelector('[class*="AlbumTracklist__Container"]');
        
        if (tracklistContainer) {
          const trackItems = tracklistContainer.querySelectorAll('li[class*="AlbumTracklist__Track"]');
          
          for (const trackItem of trackItems) {
            // Extract track number
            const trackNumberElem = trackItem.querySelector('[class*="AlbumTracklist__TrackNumber"]');
            ...
            if (link) {
              ...
              if (href) {
                const url = href.startsWith('http') ? href : `https://genius.com${href}`;
                tracklistInfo.push({ url, trackNumber });
              }
            } else {
              // No link = current song page - use the initial URL
              console.log(`Track ${trackNumber} has no link - using initial URL`);
    ...
    if (tracklistInfo.length === 0) {
      ...
      const tracklistContainer = root.querySelector('[class*="AlbumTracklist__Container"]');
    ...
    console.log('Track order:', tracklistInfo.map(t => `Track ${t.trackNumber}`).join(', '));
    ...
    for (let i = 0; i < tracklistUrls.length; i++) {
      ...
      try {
        ...
        // Extract song title
        const songTitleElem = songRoot.querySelector('h1[class*="SongHeader"][class*="Title"] span');
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to scrape album' },
      { status: 500 }
    );
  }
}

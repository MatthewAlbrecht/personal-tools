...
import { formatDate, formatPrice } from "../_utils/formatters";
...
export function ReleaseItem({
	...
	const images = useQuery(
		convexApi.folioSocietyImages.getActiveImagesByProduct,
		{
	...
	const enrichImagesAction = useAction(
		convexApi.folioSocietyImages.enrichImages,
	);
		...
		>
			<div className="min-w-0 flex-1">
				<div className="mb-2 flex items-center gap-2">
					<h3 className="truncate font-semibold text-lg">{release.name}</h3>
				</div>
			...
			<div className="ml-4 flex gap-2">
				...
				>
					<Image
						className={`h-3 w-3 ${isProcessingImages ? "animate-spin" : ""}`}
					/>

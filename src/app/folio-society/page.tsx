...
import { Database, Image, RefreshCw } from "lucide-react";
...
export default function FolioSocietyPage() {
	...
	const [searchInput, debouncedSearchInput, setSearchInput] = useDebouncedState(
		"",
		300,
	);
	...
	const processAllImagesAction = useAction(
		convexApi.folioSocietyImages.processAllImages,
	);
	...
	async function handleFullEnrichment() {
		...
		try {
			...
			toast.success(
				`Full enrichment completed! Processed ${result.attempted} products.`,
			);
		} catch (error) {
	...
	async function handleProcessAllImages() {
		...
		try {
			...
			toast.success(
				`Image processing completed! Processed ${result.processed}/${result.total} releases. Skipped ${result.skipped} already processed.`,
			);
		} catch (error) {
		...
		<div className="container mx-auto max-w-6xl p-6">
			...
			<div className="mb-6 flex items-center justify-between">
				...
				<div className="flex gap-2">
					...
					>
						<Image
							className={`h-4 w-4 ${isProcessingImages ? "animate-spin" : ""}`}
						/>

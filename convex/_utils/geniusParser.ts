...
function processLyricsHtml(html: string): string {
  ...
  text = text.replace(/<a[^>]*data-ignore-on-click-outside[^>]*>(.*?)<\/a>/gi, '$1');
  ...
  text = text.replace(/<span[^>]*style="[^"]*opacity:\s*0[^"]*"[^>]*>.*?<\/span>/gi, '');
...
export function extractAbout(html: string): string | undefined {
  ...
  const aboutRegex = /<div[^>]*class="[^"]*SongDescription__Content[^"]*"[^>]*>(.*?)<\/div>/s;
...
export function extractAlbumTracklist(html: string): string[] {
  ...
  const tracklistRegex = /<ol[^>]*class="[^"]*AlbumTracklist__Container[^"]*"[^>]*>(.*?)<\/ol>/s;
  ...
  const linkRegex = /<a[^>]*href="(https:\/\/genius\.com\/[^"]*-lyrics)"[^>]*>/g;
...
export function extractAlbumMetadata(html: string): {
  ...
  const albumTitleRegex = /<a[^>]*class="[^"]*PrimaryAlbum__Title[^"]*"[^>]*>(.*?)<\/a>/s;
  ...
  // Extract album URL
  const albumUrlRegex = /<a[^>]*class="[^"]*PrimaryAlbum__Title[^"]*"[^>]*href="([^"]+)"/;
  const urlMatch = html.match(albumUrlRegex);
  const albumUrl = urlMatch ? urlMatch[1] : '';
  
  // Extract artist name from primary artist
  const artistRegex = /<h2[^>]*class="[^"]*SongHeader[^"]*Title[^"]*"[^>]*>.*?<div[^>]*class="[^"]*HoverMarquee[^"]*"[^>]*>.*?<span[^>]*>(.*?)<\/span>/s;
  ...
  if (!artistName) {
    const fallbackArtistRegex = /<div[^>]*class="[^"]*SongHeader[^"]*CreditList[^"]*"[^>]*>.*?<a[^>]*>(.*?)<\/a>/s;
  ...
  return {
    albumTitle,
    artistName,
    albumUrl: albumUrl.startsWith('http') ? albumUrl : `https://genius.com${albumUrl}`,
  };
}
...
export function extractSongTitle(html: string): string {
  ...
  const titleRegex = /<h1[^>]*class="[^"]*SongHeader[^"]*Title[^"]*"[^>]*>.*?<span[^>]*>(.*?)<\/span>/s;
